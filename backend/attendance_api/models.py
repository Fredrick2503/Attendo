...
class section(models.Model):
    ...
    id = models.CharField(max_length=36,default=uniqueid.generateid,primary_key=True,unique=True, editable=True)
...
class class_slot(models.Model):
    id = models.CharField(default=uniqueid.generateid,max_length=36,primary_key=True,unique=True, editable=True)
    ...
    def save(self, *args, **kwargs):
        ...
        self.token = self.generate_attendance_token()
        # c=cache.set(self.token, {"class_id": self.id, "timestamp": str(self.timestamp)}, timeout=900)
        # print("cache",cache.get(self.token))
